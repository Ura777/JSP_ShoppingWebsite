				...
				try 
				{
					...
					try 
					{
						...
						else 
						{
							...
							//先找到目標會員的資料取得帳號
							sql="select * from member where id='" + request.getParameter("change2") + "'";
							temp = con.createStatement().executeQuery(sql);
							String new_account="";
							
							while (temp.next())
							{
								new_account=temp.getString(3);
							}
							...
							{
								//Step 4: 執行 SQL 指令 刪除
								sql="delete from member where id='" + request.getParameter("change2") + "';";
								con.createStatement().execute(sql);
							}
							
							//找到目標會員的Login
							sql="select * from login where account='" + new_account + "'";
							temp = con.createStatement().executeQuery(sql);
							while(temp.next())
							{
								//Step 4: 執行 SQL 指令 刪除
								sql="delete from login where account='" + new_account + "';";
								con.createStatement().execute(sql);
							}
